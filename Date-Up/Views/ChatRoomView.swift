        ...
        GeometryReader { geometry in
            ...
            VStack {
                        ...
                        ForEach(self.chatRoom.messages) { message in
                            ...
                            if message.user == profileViewModel.profile!.id {
                                ...
                                HStack {
                                    VStack {
                                        Spacer()
                                        Image(uiImage: self.chatRoom.profileLookups![0].profileImageViews[0].uiImageView.image!)
                                            .resizable()
                                            .clipShape(Circle())
                ...
                HStack {
                    ...
                    Button(action: {
                        withAnimation {
                            print("ChatRoomView: \(chatRoom.id)")
                            self.profileViewModel.sendMessageToDatabase(chatID: chatRoom.id, message: messageToBeSend) {
            ...
            .navigationBarTitle(chatRoom.profileLookups![0].profile.firstName, displayMode: .inline)
            .keyboardAdaptive()
        }
    }
}
    ...
    static var previews: some View {
        ...
        ForEach(ColorScheme.allCases, id: \.self) { colorScheme in
            ForEach(["iPhone XS MAX", "iPhone 8"], id: \.self) { deviceName in
                ChatRoomView(profile: profileViewModel, homeViewModel: homeViewModel, chatRoom: ChatRoom(id: "id", users: ["69", "96"], messages: [Message(message: "message1", picture: nil, timeStamp: Date(), user: "69"), Message(message: "message2", picture: nil, timeStamp: Date(), user: "69"), Message(message: "message3", picture: nil, timeStamp: Date(), user: "96")]))
